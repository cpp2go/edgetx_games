...
class Game {
    ...
    private computeLayout() {
        this.paddleW = Math.max(58, Math.floor(this.w * 0.16));
    ...
    private buildLevel() {
        ...
        const cols = 10;
        const gap = 3;
        const margin = Math.floor(this.w * 0.04);
        const top = Math.max(22, Math.floor(this.h * 0.08));
        const brickW = Math.floor((this.w - margin * 2 - gap * (cols - 1)) / cols);
