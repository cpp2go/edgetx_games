...
local function playSfx(freq, duration, pause)
  if settings.sound then
    playTone(freq, duration, pause or 0)
  end
end
...
local function drawProjectile(projectile, x, y)
  ...
  if projectile.travelDistance >= LCD_H then
    ...
    if currentTime >= nextShotSoundAt then
      playSfx(700, 10, 0)
...
local function isTouchEvent(event)
  return event == EVT_TOUCH_FIRST
          or event == EVT_TOUCH_SLIDE
          or event == EVT_TOUCH_TAP
...
local function renderMenu(event)
  ...
  if event == EVT_ENTER_BREAK and menuPage then -- very bad menu items handling
    ...
    if menuPosition == 5 and menuOpened then
...
local function run_func(event, touchState)
  ...
  if gameOver == true then
    ...
    if (event == EVT_ENTER_BREAK or event == EVT_TOUCH_TAP) and not menuPage then
      gameOver = false
      gameStarted = true
      hits = 0
      nextShotSoundAt = 0
